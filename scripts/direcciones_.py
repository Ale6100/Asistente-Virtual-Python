import os

usuario = os.environ.get('USERNAME') or os.environ.get('USER') # El usuario de tu PC actual

direcciones = {
    'googlemaps': {
        'palClave': ['Google Maps', 'Maps'], # Palabras clave para identificar la página
        'url': 'https://www.google.com.ar/maps',
        'buscador': 'https://www.google.com.ar/maps/search/' # Buscador
    },
    'google': {
        'palClave': ['Google', 'el buscador'],
        'url': 'https://www.google.com.ar/',
        'buscador': 'https://www.google.com.ar/search?q='        
    },
    'youtube': {
        'palClave': ['Youtube'],
        'url': 'https://www.youtube.com/',
        'buscador': 'https://www.youtube.com/results?search_query='  
    }, 
    'steam': {
        'palClave': ['Steam', 'Stim'],
        'url': 'https://store.steampowered.com/',
        'buscador': 'https://store.steampowered.com/search/?term='
    },
    'facebook': {
        'palClave': ['Facebook'],
        'url': 'https://www.facebook.com/',
        'buscador': 'https://www.facebook.com/search/top/?q='
    },
    'twitch': {
        'palClave': ['Twitch'],
        'url': 'https://www.twitch.tv/',
        'buscador': 'https://www.twitch.tv/search?term='
    },
    'twitter': {
        'palClave': ['Twitter'],
        'url': 'https://twitter.com/',
        'buscador': 'https://twitter.com/search?q='
    },
    'mercadolibre': {
        'palClave': ['MercadoLibre', 'Mercado Libre'],
        'url': 'https://www.mercadolibre.com.ar/',
        'buscador': 'https://listado.mercadolibre.com.ar/' 
    },
    'linkedin': {
        'palClave': ['LinKedIn'],
        'url': 'https://www.linkedin.com/',
        'buscador': 'https://www.linkedin.com/search/results/all/?keywords='
    },
    'github': {
        'palClave': ['GitHub'],
        'url': 'https://github.com/',
        'buscador': 'https://github.com/search?q='
    },
    'wikipedia': {
        'palClave': ['Wikipedia'],
        'url': 'https://es.wikipedia.org/',
        'buscador': 'https://es.wikipedia.org/wiki/'
    },
    'tiktok': {
        'palClave': ['Tik Tok', 'Tic Tac', 'Tic Toc'],
        'url': 'https://www.tiktok.com/',
        'buscador': 'https://www.tiktok.com/search?q='
    },
    'zoom': {
        'palClave': ['zoom'],
        'url': 'https://zoom.us/',
        'buscador': 'https://explore.zoom.us/es/search/#q='
    },    
    'amazon': {
        'palClave': ['Amazon'],
        'url': 'https://www.amazon.com/',
        'buscador': 'https://www.amazon.com/s?k='
    },
    'whatsapp': {
        'palClave': ['WhatsApp'],
        'url': 'https://web.whatsapp.com/',
    },
    'netflix': {
        'palClave': ['Netflix'],
        'url': 'https://www.netflix.com/',
    },
    'disney': {
        'palClave': ['Disney'],
        'url': 'https://www.disneyplus.com/',
    },
    'prime': {
        'palClave': ['Prime Video'],
        'url': 'https://www.primevideo.com/',
    },
    'starplus': {
        'palClave': ['Star Plus'],
        'url': 'https://www.starplus.com/',
    },
    'geogebra': {
        'palClave': ['Geogebra'],
        'url': 'https://www.geogebra.org/calculator',
    },
    'drive': {
        'palClave': ['Google Drive'],
        'url': 'https://drive.google.com/drive/u/0/my-drive',
    },
    'mercadopago': {
        'palClave': ['MercadoPago', 'Mercado Pago'],
        'url': 'https://www.mercadopago.com.ar/',
    },
    'hbo': {
        'palClave': ['HBO Max', ' Max'],
        'url': 'https://play.hbomax.com/',
    },
    'traductor': {
        'palClave': ['Traductor', 'Translate'],
        'url': 'https://translate.google.com.ar/',
    },
    'nacion': {
        'palClave': ['Banco Nacion'],
        'url': 'https://hb.redlink.com.ar/bna/login.htm',
    },
    'santander': {
        'palClave': ['Banco Santander'],
        'url': 'https://www2.personas.santander.com.ar/obp-webapp/angular/#!/login',
    },
    'buscagatos': {
        'palClave': ['Buscaminas', 'Buscagatos'],
        'url': 'https://buscagatos.netlify.app/',
    },
    'codigofuente': {
        'palClave': ['codigo fuente'],
        'url': 'https://github.com/Ale6100/Asistente-Virtual-Python.git',
    },
    'plazofijo': {
        'palClave': ['Simulador plazo fijo', 'Simulador de plazo fijo'],
        'url': 'https://simuladorplazofijo.netlify.app/',
    },
    'notas': {
        'palClave': ['notas'],
        'url': '/complementos/notas.txt',        
    },
    'canciones': {
        'palClave': ['cancion', 'musica'],
        'url': f'C:/Users/{usuario}/Music/av/musica.xspf',        
    },
    'ayuda': {
        'palClave': ['ayuda'],
        'url': f'https://github.com/Ale6100/Asistente-Virtual-Python.git#readme',        
    }
}
